...
<div class="row">
    ...
    <div class="col-sm-12">
        <?php echo form_open('master/addVendor'); ?>
        <div class="form-group">
            <label>Vendor Name</label>
            <input type="text" id="name" onkeypress="return (event.charCode > 64 && 
event.charCode < 91)|| even.charCode==36  || (event.charCode > 96 && event.charCode < 123)" value="<?=set_value('vendor')?>" name="vendor" data-validation="required" class="form-control" required>
        </div>
        <div class="form-group">
...
<div class="row">
    ...
    <div class="col-sm-12">
        <?php echo form_open('master/addVendor/'.base64_encode($details['id'])); ?>
        <div class="form-group">
            <label>Vendor Name</label>
            <input type="text" id="name" onkeypress="return (event.charCode > 64 && 
event.charCode < 91)|| even.charCode==36   || (event.charCode > 96 && event.charCode < 123)" value="<?=$details['vendor_name']?>" name="vendor" data-validation="required" class="form-control" required>
        </div>
        <div class="form-group">
        <label for="exampleInputPassword1">Contact No.</label>
        ...
        <div class="form-group">
            <br>
            <a href="<?=base_url('master/vendor')?>" class="btn btn-primary rounded-0">Back</a>
            <button class="btn btn-success rounded-0" type="submit" name="edit">Save</button>
            <a href="<?=base_url('master/deleteVendor/')?><?=base64_encode($details['id'])?>" onclick="return confirm('Are you sure?')" data-toggle="tooltip" data-placement="top" class="btn btn-danger rounded-0" data-original-title="Delete">Delete</a>
        </div>
        <?php echo form_close(); ?>
        ...
        <tbody>
            <?php $i=0; foreach($vendors as $vendor): ?>
                <tr>
                    ...
                    <td>
                        <a href="<?=base_url('master/vendor/')?><?=base64_encode($vendor['id'])?>" data-toggle="tooltip" data-placement="top" class="text-success" data-original-title="Edit"><i class='fas fa-pencil-alt'></i></a>
                        &nbsp&nbsp&nbsp&nbsp&nbsp
                        <a href="<?=base_url('master/deleteVendor/')?><?=base64_encode($vendor['id'])?>" onclick="return confirm('Are you sure?')" data-toggle="tooltip" data-placement="top" class="text-danger" data-original-title="Delete"><i class='fas fa-times'></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
